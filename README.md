...
**Status: v0.1.0-alpha.1 — experimental, shadow mode only.**
...
- Bounded event context by count, age and serialized input character budget.
...
- Explicitly enabled HA WebSocket observation with entity aliases; no service-call implementation.
...
The provided demo service has no network, host ports or host volumes. Building downloads dependencies. The container recipe has not yet been runtime-validated; the Python path is the tested baseline.
